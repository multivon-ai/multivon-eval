...
# Field aliases — common trace dumps from LangSmith / LangFuse / Phoenix / OTel use
# different key names for the same concept. We auto-rename so the downstream
# evaluator pipeline sees a canonical shape, and we emit a loud summary of how
# many rows we renamed vs skipped (the original silent-skip behavior caused real
# users to think their dump was empty when 198/200 rows had used `query` instead
# of `input`).
_INPUT_ALIASES = ("input", "query", "question", "prompt", "user_input", "user_message")
...
def load_traces(
    ...
    *,
    ...
    """Read a JSONL file of traces, auto-renaming common field aliases.

    Accepts trace dumps from LangSmith (``query``/``answer``), LangFuse
    (``prompt``/``completion``), Phoenix (``input``/``output``), and OTel.
    A row that names its prompt field ``query`` is treated identically to
    one that names it ``input`` — the field is renamed in place to the
    canonical key.

    When ``verbose`` is True (the default), prints a one-line summary to
    stderr describing how many rows loaded, how many were renamed, and
    ...
    for i, line in enumerate(p.read_text(encoding="utf-8").splitlines(), 1):
        ...
        # Auto-rename aliases (first-match-wins, preserves canonical if present).
    ...
    if verbose:
        ...
        if any(renamed.values()):
            ...
            parts.append(f"renamed alias fields ({details})")
        if skipped_no_input > 0:
            parts.append(
                f"skipped {skipped_no_input} rows with no input/query/prompt/question field"
            )
        print("[load_traces] " + " · ".join(parts), file=sys.stderr)
...
This is a runnable starting point — `python eval_suite.py` will load the
adversarial seed cases bootstrap emitted next to this file and run them
against a stub model. Replace ``stub_model`` with your real model
function (or pass ``--cases path/to/real.jsonl`` to point at your own
case file) before shipping.
...
def stub_model(prompt: str) -> str:
    ...
    (Anthropic/OpenAI/local/etc.) before shipping. Without that swap
    every case will return the same stub string and the suite will
    flag everything as failing, which is the intended signal that
    you haven't wired your model yet.
    """
    return "[stub model — replace stub_model() in eval_suite.py with your real model call]"


def main() -> int:
    ap = argparse.ArgumentParser(description="Run the bootstrap-generated eval suite.")
    here = Path(__file__).resolve().parent
    ap.add_argument(
        "--cases",
        type=Path,
        default=here / "seed_cases.jsonl",
        help="Path to JSONL cases file (default: seed_cases.jsonl next to this script).",
    )
    ap.add_argument(
        "--runs",
        type=int,
        default=1,
        help="Number of times to run each case (multi-run flakiness detection).",
    )
    args = ap.parse_args()
