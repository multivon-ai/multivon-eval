...
def _render_eval_suite_py(shape: ProductShape, evaluators: list[RecommendedEvaluator]) -> str:
    ...
    eval_lines = []
    for r in evaluators:
        # Most evaluators accept threshold as a kwarg; SchemaEvaluator + a few
        # take other shapes. We emit threshold=... and let runtime errors
        # surface if a class doesn't support it.
        eval_lines.append(f"    {r.name}(threshold={r.threshold}),  # {r.tier}: {r.rationale[:80]}")
...
{discussion or "No additional discussion provided by the proposer model."}
