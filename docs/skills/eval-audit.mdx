...
`eval-audit` is the pre-ship local check. For post-merge runs, scheduled nightly suites, and PR comment automation, pair it with [`eval-action`](https://github.com/multivon-ai/eval-action) — the GitHub Action that runs the full suite on push, posts a diff comment to the PR, and (optionally) blocks merge on safety regressions. The two share the same `multivon-eval` machinery; the skill is local-fast-targeted, the Action is CI-thorough-comprehensive.
