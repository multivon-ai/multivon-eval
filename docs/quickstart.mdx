...
Returns four files: `eval_suite.py` (runnable), `seed_cases.jsonl` (30 adversarial cases), `thresholds.yaml` (calibrated from your traces), and `DISCOVERY_REPORT.md` (an eval design review). Cost: ~$0.12 default (or free with `--judge-provider ollama`). **[Full walkthrough →](/guides/bootstrap)**
