...
report = suite.run(lambda prompt: "Paris", verbose=False, fail_threshold=1.0)
assert report.passed == 1 and report.errors == 0
```

Replace the lambda with your application and write cases for the task you
actually need it to complete. See [Define task success](/guides/task-success).

## Propose a suite with `multivon-eval bootstrap`

Don't know what to eval for your specific LLM product? Describe it and hand over a few sample traces. `multivon-eval bootstrap` proposes a starter suite in a few minutes.
...
Returns five files: `eval_suite.py` (runnable), `seed_cases.jsonl` (up to 30 accepted synthetic cases), `thresholds.yaml` (provisional p25 score thresholds to review), `DISCOVERY_REPORT.md` (an eval design review), and `prompt_baseline.json` (a prompt call-site baseline written at the repo root for staleness tracking). Cost depends on judge calls and pricing; local Ollama avoids hosted API charges. Threshold suggestions are not validated acceptance criteria. **[Full walkthrough →](/guides/bootstrap)**
...
Check the installation without contacting a provider:

```bash
multivon-eval doctor --no-ping --json
```

`doctor` exits 0 when every check is clean, 2 when it finds warnings (for
example, no optional provider key), and 1 when it finds an error. An exit code
of 2 is diagnostic and does not mean the package failed to install.
