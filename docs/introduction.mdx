---
title: Introduction
description: AI evaluation for teams that ship models to production.
---

**multivon-eval** is an open source Python SDK for evaluating AI outputs — from simple string checks to LLM judges to agent trace analysis.

```bash
pip install multivon-eval
```

## What it does

<CardGroup cols={2}>
  <Card title="Deterministic" icon="bolt">
    String matching, regex, JSON schema, BLEU, ROUGE, latency. Instant, free, no LLM.
  </Card>
  <Card title="LLM-as-judge" icon="brain">
    QAG scoring — yes/no questions instead of unreliable 1-10 ratings. Faithfulness, hallucination, relevance, and more.
  </Card>
  <Card title="Agent trace" icon="route">
    Tool call accuracy, plan quality, step faithfulness, task completion. Framework-agnostic.
  </Card>
  <Card title="Conversation" icon="messages">
    Knowledge retention, relevance, consistency, and completeness across multi-turn sessions.
  </Card>
</CardGroup>

## Key features

**QAG scoring** — Instead of asking a judge "rate this 1-10", we generate binary yes/no questions about the output and score by fraction answered correctly. [Benchmarked at 65% fewer false positives](https://github.com/multivon-ai/multivon-eval/tree/main/benchmarks) than numeric scoring with the same judge model.

**No cold start** — Point `generate_from_file()` at your docs and get eval cases in seconds. No labeled dataset required to get started.

**Experiment tracking** — Record every run to `~/.multivon/experiments/`. Compare two runs side-by-side and get a pass rate delta. Catch regressions before they ship.

**CI/CD first** — One line exits with code 1 if your pass rate drops below a threshold. Evals that don't run in CI catch nothing.

## Install

```bash
pip install multivon-eval
```

Requires Python 3.10+. Set `ANTHROPIC_API_KEY` and/or `OPENAI_API_KEY` for LLM-judge evaluators.

<CardGroup cols={2}>
  <Card title="Quickstart" icon="rocket" href="/quickstart">
    Up and running in 5 minutes
  </Card>
  <Card title="Generate datasets" icon="wand-magic-sparkles" href="/guides/generate">
    No labeled data? Generate cases from your docs
  </Card>
</CardGroup>
