...
`multivon-mcp` 0.3.2 is a [Model Context Protocol](https://modelcontextprotocol.io) server that exposes 22 evaluation tools to any MCP-compatible agent — Claude Desktop, Claude Code, Cursor, Cline, OpenCode. The agent calls them by name; no copy-paste, no `python -c "..."`, no asking the agent to figure out the SDK calls.
...
<Note>
The current 0.3.2 package predates MCP Python SDK 2.0. Until the next
multivon-mcp release publishes its `mcp&lt;2` constraint, install with
`pip install "mcp&lt;2" multivon-mcp` to avoid resolving the incompatible 2.0
server API.
</Note>
