...
Deterministic agent tool-call correctness. The released 0.3.2 protocol supports
single-call name and optional argument-dict comparison. The additive trace mode
below is on `multivon-mcp` main and will ship in its next release. **No LLM judge.**

| Arg | Type | Default |
|---|---|---|
| `expected_tool` | `str` | required in single-call mode |
| `actual_tool` | `str` | required in single-call mode |
| `expected_arguments` | `dict` | `None` |
| `actual_arguments` | `dict` | `None` |
| `expected_tool_calls` | `list[str]` | required in trace mode; `[]` asserts no calls |
| `agent_trace` | `list[dict]` | canonical steps returned by `eval_ingest_trace` |
...
Build a procurement-ready ZIP with a SHA-256 manifest over every bundled file.
Anchor the manifest hash outside the ZIP for evidence against deliberate
rewriting. **No API calls.**
...
`eval_discover` returns the full 44-evaluator catalog so the agent can always introspect everything. The 22 directly exposed are the ones agents actually call mid-edit:
