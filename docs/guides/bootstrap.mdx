...
description: Cold-start your evals — `multivon-eval bootstrap` proposes a starter suite from a product description + sample traces in a few minutes.
---
...
| `seed_cases.jsonl`        | Up to 30 accepted synthetic cases targeting a suggested failure mode        |
| `thresholds.yaml`         | Provisional thresholds from your traces (p25 of measured scores) |
...
| `expected_output`        | Enables reference-dependent metrics such as AnswerAccuracy / ContextRecall |
...
Generation may use multiple batches and trace-side scoring may use many calls.
A p25 threshold describes the current score distribution: it does not learn
which outputs humans consider acceptable. Do not use these suggestions as a
production acceptance policy until you have reviewed labeled examples, tuned
on a development split, and checked a separate held-out split.
...
| `--skip-seed-cases`  | off                              | Skip seed generation calls          |
...
result = bootstrap(
    ...
    pii_policy="redact",
    n_seed_cases=30,
    repo=".",  # include prompt_baseline.json; Python defaults to no repo scan
)
