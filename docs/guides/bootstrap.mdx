...
- [`auto_evaluators(case)`](/evaluators/llm-judge#auto-evaluators) — the deterministic heuristic anchor under the bootstrap. Microseconds, no LLM call.
- [`generate_adversarial_cases`](/guides/synthetic-data) — the failure-mode generator the bootstrap uses for seed cases. Targets 10 named failure modes (`ungrounded_claim`, `jailbreak`, `prompt_injection_direct`, etc.).
- [`validate_adversarial_cases`](/guides/synthetic-data) — N-shot judge-noise filter for adversarial cases. Validated +0.80 mean failure-rate separation between weak and strong baselines.
