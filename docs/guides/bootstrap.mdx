...
Total cost: ~$0.12 per bootstrap on default settings (or **$0.00 with `--judge-provider ollama`**). Latency: under 60 seconds for default settings; ~5× wall-clock for the local-judge path.

As of 0.9.4 the emitted `eval_suite.py` is genuinely runnable end-to-end — `python eval_suite.py --runs 1` executes the full suite without further edits. (0.9.4 also fixed a stale `suite.run(cases=...)` kwarg and `report.print_summary()` call that previously broke the generated file; if you hit those on a 0.9.3-and-earlier install, upgrade.)
