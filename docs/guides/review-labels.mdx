...
**Development preview on `main`; not included in PyPI 0.18.0.** The JSON
adapter has no extra runtime dependencies. Its configuration and generated
annotations were checked with Label Studio SDK 2.1.1; a deployed-server UI
round trip has not yet been validated.
...
saved-score threshold-fitting and held-out analysis workflow is still being
implemented. Do not present the existing historical threshold packs as evidence
