...
# Needs cases that carry both `context` and `expected_output` — a twin is
# a minimally-edited UNFAITHFUL answer against the same context. Cases
# without context are skipped (and counted in the report).
```
