...
Pass `fail_threshold` to `suite.run()`. A completed quality failure raises
`EvalGateFailure` with exit code 1. Since 0.17.0, an active gate defaults to
zero tolerated errors and blocks empty or skipped coverage with exit code 2
(indeterminate). Set `max_error_rate` explicitly only if you intend to permit
an error budget. A high pass rate alone does not establish sufficient coverage.
...
The built-in gate blocks wholly skipped cases. Use an [acceptance policy](./acceptance-policies)
for required checks and per-slice coverage.
...
## Apply a saved release contract

```bash
multivon-eval gate eval_results.json --policy evals/acceptance.json --output decision.json
```

This uses the same versioned policy as `eval_acceptance_report` in
multivon-mcp 0.4.0. The corresponding eval-action v2 update is prepared but
pending publication; check its release notes before choosing an action version. The policy returns accept/reject/indeterminate
with exit codes 0/1/2; it does not require a baseline or a p-value.
...
This returns 0 for complete, paired results with no regressions, 1 for a
quality regression, and 2 for incomplete evidence (including errors, skips,
or unmatched cases). It does not prove equivalence or require a significant
p-value before flagging a case regression.
