...
report as UNVERIFIABLE, never orphaned.
...
- **Runtime recorder** — an opt-in pytest plugin capturing rendered prompts
  at eval time; the honest v2 mechanism for case→site binding and for
  verifying dynamic prompts. Part of the staleness epic,
  [multivon-eval#4](https://github.com/multivon-ai/multivon-eval/issues/4).
...
- [Prompt attribution](/guides/attribution) — the scanner this is built on,
  including the scanner v2 changes (one-hop constant resolution,
  `loose_fingerprint`).
