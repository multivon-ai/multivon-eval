...
Everything else — `Name` references, `Attribute` lookups, runtime
...
assembled strings, or named constants used by reference elsewhere. If the
prompt isn't a literal kwarg at a recognized SDK call site, it doesn't
land in the diff. This is deliberate — the v1 adversarial-fix discipline
dropped fuzzy name-regex capture entirely.
</Warning>
