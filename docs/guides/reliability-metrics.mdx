...
The gap distinguishes occasional success from repeated success on the sampled tasks. Neither metric alone establishes a production SLA.

---

## The estimators

Both are computed per task from its n recorded trials (c of which passed), then averaged over tasks. Their interpretation as probabilities for future trials assumes comparable, independent sampling within a task. Caching, shared state, and adaptive retries can violate that assumption.
...
The confidence intervals are a **cluster bootstrap**: tasks are resampled with replacement, the mean of per-task estimators is recomputed, and a percentile interval is taken. Trials within a task are correlated — the same prompt, the same failure modes — so resampling raw trials would treat 20 tasks × 5 trials as 100 independent observations and fake precision the data doesn't have. The implementation treats tasks as independent. If several cases share a source document, conversation, or customer, this interval does not account for that larger cluster.

Degenerate suites (every per-task estimate identical, including all-pass) fall back to a Wilson interval on the mean, so a perfect score still reports `ci_low < 1.0`. For fractional per-task estimates this is a heuristic fallback, not an exact binomial coverage guarantee.
...
Use these cases to target debugging. Current aggregated reports do not retain every trial transcript; capture trial outputs separately in your application if you need to compare passing and failing attempts.
