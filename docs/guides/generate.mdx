...
Generate transformation candidates from existing cases. In the development
preview, every candidate clears its expected answer and records an unverified
`invariant` or `flip` hypothesis with `oracle_status="unknown"`. Even whitespace
or punctuation changes can alter the correct answer. Validate candidates with
[task-specific oracles](/guides/controlled-robustness) before scoring them.
The report's accepted count describes structural generation, not oracle validity.
This replaces the label-copying behavior in PyPI 0.18.0.
