...
`ComplianceReporter` writes a hash-chained, tamper-evident NDJSON log of every eval run, with paragraph-accurate regulatory control annotations on each evaluator result.
...
```python
ComplianceReporter(framework="eu-ai-act")    # paragraph-accurate EU AI Act controls
