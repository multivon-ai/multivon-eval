...
```python
from multivon_eval import EvalSuite, Experiment, load
...
# After
report_v2 = suite.run(rag_pipeline_v2, save_json="proposal.json")
...
**Illustrative quality change (not a measured result):**
```
Pass rate   91.0%  →  74.0%  ↓  -17.0%
faithfulness  0.8900  →  0.6800  ↓  -0.2100
```

This comparison is exploratory. To block deployment, configure a quality gate
and preserve failure artifacts using the [CI/CD pattern](/guides/ci-cd). Inspect
retrieval and grader reasons before attributing a drop to chunk size.
