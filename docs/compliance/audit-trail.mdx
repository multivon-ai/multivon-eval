...
- `record_id` — 12-character hex (truncated from `uuid4().hex`). Sufficient for cross-referencing within a single suite's log; use it together with `timestamp` and `suite_name` for global uniqueness. Not a UUID, and calling it one would be inaccurate.
...
- **OK (legacy)** — record predates `chain_version=1` (no `chain_version` field). Verified standalone; the chain link is not checked because the legacy format didn't include `prev_hash`. Surfaced explicitly so the auditor knows the chain coverage is partial.
