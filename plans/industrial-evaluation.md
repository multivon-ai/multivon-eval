...
| R03 | Resumable execution and resource controls | Kill/restart experiment preserves completed work; checkpoint compatibility, cancellation, bounded concurrency, deadlines and budget accounting; explicit policy for ambiguous side effects | In progress: Inspect SIGKILL/recovery preserves completed work and rejects duplicate writes; native async ownership fixes and upstream limit/termination evidence added. Offline deadline, usage-limit, concurrency and cancellation protocol covers a bounded profile; full checkpoint compatibility and production resource guarantees remain. See [control protocol](execution-controls.md) |
