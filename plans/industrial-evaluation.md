...
| R02 | Complete immutable trial evidence and regrading | All outputs, traces, grader results, retries/errors, effective requests, usage retained across sync/async/parallel paths; saved output regrading makes no target calls | In progress: per-run/retry snapshots, strict agent judgments and instrumented native HTTP request/usage evidence; SQLite SIGKILL retention tested. Unobserved transports, complete target configuration and coverage-aware accounting remain; see [provider protocol](provider-evidence.md) |
