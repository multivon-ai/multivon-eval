...
- **`validate` can no longer report green when zero graders executed.** A judge-only suite audited in the default offline mode skipped every grader yet landed every case `OK`, `passed=True`, and counted them informative. A case where no grader produced a verdict is now `UNVALIDATABLE` ("all graders are judge-backed; rerun with --judges"), and a report where zero graders executed overall gets the report-level status `NOTHING_VALIDATED` — `passed` is False, the CLI exits 1, and `__str__` says so plainly.
- **`validate`'s contrast discrimination check is guaranteed judge-free offline.** The contrast rerun now explicitly excludes judge-backed graders unless `include_judges=True` (whose spend was already tracked); the docstring and `docs/guides/validate.mdx` "zero LLM calls" claim now match the enforced behavior. `ValidationReport.to_dict()` / `to_json()` gained the `costs` and `status` fields plus a `skipped_judge_graders` disclosure; `__str__` discloses skipped graders and judge spend.
- **`validate` routes judge outages to UNVALIDATABLE, not BROKEN.** With `--judges`, a `JudgeUnavailable` from a grader marked the case `BROKEN_TASK_OR_GRADER`; a judge outage is infrastructure, not a grader verdict, and now lands `UNVALIDATABLE` with an infra-flavored reason. Only genuine grader verdicts may say BROKEN.
- **`_parse_yes_no` verdict prefixes are word-bounded.** Bare `startswith` matched "Yesterday was unclear" as YES and "Nobody can determine this" as NO; leading verdicts now require the WORD yes/no (`^\W*(yes|no)\b`), the single-verdict-token fallback unchanged. Also added a minimum-verdict-coverage rule: when more than half of a QAG evaluator's questions (or Faithfulness claims) parse UNKNOWN, the evaluator raises `JudgeUnavailable` (→ `JUDGE_ERROR` status, consistent with the existing all-UNKNOWN behavior) instead of scoring the parseable remainder — 1 YES + 9 UNKNOWN no longer scores 1.0. Coverage between zero and half stays scored, disclosed in the reason.
- **`early_stop` on the parallel path warns instead of silently no-opping.** `workers` defaults to `min(8, len(cases))`, so `early_stop=True` without `workers=1` never reached the SPRT loop; the run now emits "early_stop requires workers=1; ignoring" on stderr and the `run()` docstring says so.
- **`suite_pass_k` validates its arguments** — `confidence` must be in (0, 1) exclusive and `n_boot >= 1`, else `ValueError`. The degenerate-CI Wilson fallback is now labeled in its docstring as a conservative heuristic, not a bootstrap-consistent interval.
- **Docs honesty:** `docs/guides/anthropic-evals-checklist.mdx` rows for pass@k/pass^k, the saturation monitor, and judge-UNKNOWN now carry the "(unreleased — on main)" marker their sibling validate row already had; `docs/guides/validate.mdx`, the validate CLI's file-loading error text, and the README quickstart snippet document the `if __name__ == "__main__":` guard (an unguarded module-level `suite.run()` executes the model at import time — the never-calls-your-model guarantee needs the guard).
- **Judge verdict parser loses its yes-bias and gains an UNKNOWN out.** `_parse_yes_no` fell back to `"yes" in text[:50]`, so a hedged judge reply like "I cannot say yes or no with certainty" counted as YES. Clear prefix verdicts parse exactly as before (calibrated thresholds were fit under them); replies with no unambiguous verdict now return UNKNOWN — excluded from the QAG score denominator and disclosed in the reason (e.g. "1 of 3 question(s) UNKNOWN"). A case where *every* verdict is unparseable raises `JudgeUnavailable` → `JUDGE_ERROR` status instead of scoring 0/0. Applies to all QAG evaluators plus the agent evaluators that share the parser.
- **QAG evaluators no longer launder judge errors into quality scores.** Per-question judge exceptions were scored as silent False votes and a failed claim extraction returned score 0.0 ("Failed to extract claims") — indistinguishable from a model failure. Exceptions now propagate to the suite's status taxonomy (`JudgeUnavailable` → `JUDGE_ERROR`, anything else → `EVALUATOR_ERROR`, both excluded from `pass_rate`). Faithfulness claim extraction is JSON-first with a greedy-bracket fallback (claim strings containing `]` no longer truncate the list), and the 10-claim verification cap is disclosed in the reason ("verified 10 of 25 claims (capped)").
- **`resolve_judge` merge uses None-sentinels.** The override merge compared against default values (`temperature != 0.0`, `max_tokens != 1024`, `timeout != 30`, `reliability_sample != 5`), so an explicit `JudgeConfig(temperature=0.0)` could never override a nonzero global. Those fields now default to `None` (= inherit) and resolve to the same effective defaults as before; explicit values — including 0.0 — always win.

### Added

- **pass@k / pass^k reliability metrics from `--runs N` data.** New `multivon_eval/passk.py` + `EvalReport.pass_at_k(k)` ("at least one of k trials passes" — capability, unbiased HumanEval combinatorial estimator) and `EvalReport.pass_hat_k(k)` ("all k trials pass" — what a user hitting the feature k times experiences; exact hypergeometric estimator, never the upward-biased `(c/n)**k` plug-in). Both return a `PassKResult` with a cluster-bootstrap CI that resamples CASES, not trials (trials within a case are correlated — resampling them would fake precision), with a Wilson fallback when the bootstrap degenerates so an all-pass suite still reports `ci_low < 1.0`. `k > runs` is an honest UNKNOWN (`value is None` plus a rerun hint) — no extrapolation. `EvalReport.lottery_cases()` ranks the cases driving the pass@k↔pass^k gap; `EvalReport.assert_pass_hat_k(k, min_ci_low)` gates CI on the pass^k CI *lower bound* and fails loudly (never silently passes) when the metric is UNKNOWN. Multi-run terminal reports print a reliability block; `compare` and `view --dir` surface pass^k.
- **Saturation monitor** — a suite at 100% can no longer detect improvement, and the report now says so instead of celebrating. New `EvalReport.saturated` (all EVALUATED cases passed — a 100% built on judge outages doesn't count) and `EvalReport.min_detectable_regression` (smallest pass-rate drop detectable at 80% power, variance anchored near the observed rate and capped at baseline 0.95 so a perfect score can't flatter its own sensitivity). New `EvalSuite(purpose=)` in `{'', 'capability', 'regression'}`, copied onto the report and serialized in the JSON summary alongside `saturated` / `min_detectable_regression`. Terminal reporter: a saturated capability suite (n ≥ 3) gets a one-line warning quantifying the Wilson floor + minimum detectable regression and recommending graduation to a regression suite plus harder tasks; a `purpose='regression'` suite inverts — any task below ceiling prints a triage warning instead. Always a WARNING, never a gate. `view --dir` shows a `saturated` badge.
- **`multivon-eval validate` — grade your graders.** New `multivon_eval/validate.py` + CLI subcommand + `suite.validate()`: for every case with a reference output (new additive `EvalCase.reference_output`, falling back to `expected_output`; callables invoked at validate time, exceptions → BROKEN), run the suite's evaluators against the REFERENCE. A reference that fails its own graders is `BROKEN_TASK_OR_GRADER` — the task is unsolvable or the grader miscalibrated; either way the agent is innocent. Never calls the model under test. LLM-judge evaluators (new `Evaluator.uses_llm_judge` flag) are skipped by default; `--judges` opts in and reports spend. Cases linked to a contrast twin (`metadata['pair_id']`) get a zero-LLM-call discrimination check: a grader passing both the reference and the twin's known-bad output is `NO_DISCRIMINATION` ("effective informative cases: N/M validated"). Cases without any reference are `UNVALIDATABLE` with a nudge, never dropped. CLI exits 1 only on BROKEN — warnings don't break CI. `reference_output` is deliberately EXCLUDED from the lockfile `cases_hash`, so historical suite locks are unaffected (regression-tested against a pinned pre-change hash).
- **Zero-pass suspect detection** — `EvalReport.zero_pass_cases`: evaluated cases that failed every trial with `runs > 1`, or every failing case when a single-run suite lands at exactly 0%. The terminal report footer flags them: 0% pass usually means a broken task or grader, not an incapable agent — run `multivon-eval validate` before blaming the model.
- **`max_error_rate` error budget on the CI gate** (`suite.run` / `run_async` / `run_on_cases`). `pass_rate` excludes errored cases by design, so 90 judge errors + 10 passes gated green. With `max_error_rate` set, the gate raises `EvalGateFailure` ("Eval gate INDETERMINATE: …") when `errors/total` exceeds the budget. Unset, a `fail_threshold` gate warns loudly on stderr at ≥ 10% errors — the same threshold `view --dir` already flags. New `EvalReport.error_rate` property.
