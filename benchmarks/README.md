...
| Do open-weights specialist judges already address the in-house judge thesis? | [External judges, partial](results/external_judges.json): HHEM-2.1-open reaches F1 0.644 at its vendor cut and 0.674 swept on 100 HaluEval QA pairs, below the 0.72 pivot trigger — but the bootstrap intervals reach 0.75/0.78, so n=100 does not exclude it, and the in-house comparator is a stale number the README already flags for revalidation. Two hosted judges remain unmeasured for want of keys. |
